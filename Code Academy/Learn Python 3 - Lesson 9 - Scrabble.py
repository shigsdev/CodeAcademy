'''Scrabble In this project, you will process some data from a group of friends playing scrabble. You will use
...
# letter_to_points = dict(list(zip(letters,points)))
letter_to_points = {k: v for k, v in zip(letters, points)}

print(letter_to_points)

# Our letters list did not take into account blank tiles. Add an element to the letter_to_points dictionary that has
# a key of " " and a point value of 0.
...
#
# 5. After defining point_total, create a for loop that goes through the letters in word and adds the point value of
...
player_to_words = {'player 1': ["BLUE", 'TENNIS', 'EXIT'], 'wordNerd': ['EARTH', 'EYES', 'MACHINE'],
...
# Within your loop, create a variable called player_points and set it to 0. Within the loop, create another loop that
# goes through each word in words and adds the value of score_word() with word as an input. After the inner loop
# ends, set the current player value to be a key of player_to_points, with a value of player_points.
...
play_word('player 1', 'ASSHOLE')
play_word('player 3', 'legend')
print(player_to_words)
print(player_to_points)
